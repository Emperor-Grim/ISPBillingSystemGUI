...
/** database palagay
 CREATE TABLE bills (
    ...
    FOREIGN KEY (customerId) REFERENCES customers(id) ON DELETE CASCADE  -- Fix #10: cascade so deleting a customer also removes their bills
 );
 */
public class ISPBillingSystem extends javax.swing.JFrame {
    ...
    //----------------------DASHBOARD MODIFICATION CUSTOM----------------------------------    
        ...
        custTable = styledTable(custmodel);  // Fix #1: was styledTable(model) — wrong shared model
        custScrollPane.setViewportView(custTable);
        styleStatusColumn(custTable, 5);
        
              
        // Fix #2a: included dueDay (7th column) + table will be refreshed via refreshDashboardTable()
    ...
    // Fix #4: new method — keeps the dashboard customer table in sync after every CRUD
    ...
    void refreshBillTable() {
        // Fix #3: removed duplicate dashboard stat update (refreshDashboard() handles that)
    ...
    private void searchFieldActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_searchFieldActionPerformed
        ...
        for (Customer c : dao.getAllCustomers()) {
            if (c.getFullName().toLowerCase().contains(query) ||
                ...
                m.addRow(new Object[]{
                    c.getId(), c.getFullName(), c.getAddress(),
                    c.getPlan(), String.format("₱%.2f", c.getBalance()), c.getStatus(), c.getDueDay()  // Fix #5: added dueDay
                });
            }
        }
    ...
    private void generateReceiptBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_generateReceiptBtnActionPerformed
        ...
        // Fix #6: look up customer by ID (from bill row) — avoids duplicate-name bug
        int billId      = (int) billTable.getValueAt(row, 0);
        String period   = billTable.getValueAt(row, 2).toString();
        String amount   = billTable.getValueAt(row, 3).toString();

        // Get the customerId from the bill record via DAO
        Bill selectedBill = null;
        for (Bill b : billDao.getAllBills()) {
            if (b.getId() == billId) { selectedBill = b; break; }
        }
        if (selectedBill == null) return;

        Customer selectedCustomer = dao.getCustomerById(selectedBill.getCustomerId());
